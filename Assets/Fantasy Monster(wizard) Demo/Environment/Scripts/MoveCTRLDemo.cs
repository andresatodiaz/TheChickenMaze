...
public class MoveCTRLDemo : MonoBehaviour {
	...
	private float healingTimer=1.5f;
	...
	void OnCollisionEnter(Collision other)
    {
		if(other.transform.CompareTag("Player")){
			...
			if(player.GetComponent<playerMovement>().health<1f){
				player.GetComponent<playerMovement>().health=1f;
			}

			audioSrc.PlayOneShot(Healing,1f);
